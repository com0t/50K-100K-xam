...
class WPANALYTIFY_Utils {
	...
	*/
	 ...
	 * @return boolean
	 */
	public static function is_tracking_available() {

		$track = false;
		
		if ( get_option( 'pa_google_token' ) ) { // Authenticated, check profiles selection and tracking option
			if ( WP_ANALYTIFY_FUNCTIONS::get_UA_code() && ! empty( $GLOBALS['WP_ANALYTIFY']->settings->get_option( 'profile_for_posts', 'wp-analytify-profile' ) ) ) { 
				$track = true;
			}
		} else if ( $GLOBALS['WP_ANALYTIFY']->settings->get_option( 'manual_ua_code', 'wp-analytify-authentication', false ) ) { // Not authenticated, check manual code
			$track = true;
		}
